...
const form = ref({
  id: 0, firmId: null, jobId: null, contractNo: '', startDate: '', endDate: '', documentPath: ''
})
...
function openAdd() {
  form.value = {
    id: 0, firmId: firms.value[0]?.id ?? null, jobId: null, contractNo: '',
    startDate: new Date().toISOString().slice(0, 10), endDate: '', documentPath: ''
  }
  showForm.value = true
}
function openEdit(row) {
  form.value = {
    id: row.id, firmId: row.firmId, jobId: row.jobId ?? null, contractNo: row.contractNo,
...
async function save() {
  ...
  try {
    const payload = {
      ...
      documentPath: form.value.documentPath || null,
    }
  ...
  <div class="page">
    ...
    <div v-if="showForm" class="modal" @click.self="showForm = false">
      <div class="modal-content">
        <h3>{{ form.id ? 'Düzenle' : 'Yeni Sözleşme' }}</h3>
        <form @submit.prevent="save">
          ...
          <label>İş (opsiyonel) <select v-model.number="form.jobId"><option :value="null">—</option><option v-for="j in jobs" :key="j.id" :value="j.id">#{{ j.id }} {{ j.site?.name }}</option></select></label>
          <label>Sözleşme no <input v-model="form.contractNo" required /></label>
          <label>Başlangıç <input v-model="form.startDate" type="date" required /></label>
          <label>Bitiş <input v-model="form.endDate" type="date" required /></label>
          <label>Döküman yolu <input v-model="form.documentPath" placeholder="/uploads/..." /></label>
