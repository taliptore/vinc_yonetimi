...
const cranes = ref([])
const form = ref({ id: 0, fullName: '', phone: '', email: '', licenseType: '', craneId: null })
const saving = ref(false)

async function load() {
  loading.value = true
  try {
    const [r1, r2] = await Promise.all([api.get('/operators'), api.get('/cranes')])
    list.value = r1.data
    cranes.value = r2.data
  } finally {
    loading.value = false
  }
}
...
function openAdd() {
  form.value = { id: 0, fullName: '', phone: '', email: '', licenseType: '', craneId: null }
  showForm.value = true
}
...
async function save() {
  saving.value = true
  try {
    if (form.value.id) await api.put(`/operators/${form.value.id}`, form.value)
    else await api.post('/operators', form.value)
  ...
  <div class="page">
    ...
    <div v-if="showForm" class="modal" @click.self="showForm = false">
      <div class="modal-content">
        <h3>{{ form.id ? 'Düzenle' : 'Yeni Operatör' }}</h3>
        <form @submit.prevent="save">
          <label>Ad Soyad <input v-model="form.fullName" required /></label>
          <label>Telefon <input v-model="form.phone" /></label>
          <label>E-posta <input v-model="form.email" type="email" /></label>
          <label>Ehliyet tipi <input v-model="form.licenseType" /></label>
          <label>Bağlı vinç
            <select v-model.number="form.craneId">
